...
import { Sparkles, X, Megaphone, CheckCircle2, AlertCircle } from "lucide-react";
...
function FloatingActions({ buttons }) {
  ...
  return (
    <div className="fixed bottom-16 right-4 z-40 flex flex-col gap-2.5 items-end max-w-[200px]">
      {buttons.map((btn) => (
        <a
          key={btn.id}
          href={btn.link || "#"}
          ...
          className="group px-4 py-2.5 bg-neutral-900/90 backdrop-blur-md border border-neutral-800 text-[#d9b04f] rounded-full text-xs font-bold shadow-lg hover:shadow-xl hover:scale-105 hover:bg-neutral-850 active:scale-95 transition-all duration-200 flex items-center gap-2 cursor-pointer max-w-[180px] truncate"
        >
          <Sparkles className="w-3.5 h-3.5 text-[#d9b04f] shrink-0 animate-pulse group-hover:rotate-12 transition-transform" />
          <span className="truncate text-white font-medium group-hover:text-[#d9b04f] transition-colors">{btn.label}</span>
        </a>
      ))}
    </div>
  );
