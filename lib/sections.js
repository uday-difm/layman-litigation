...
function HeroSection({ content }) {
  ...
  return (
    <section className="relative w-full min-h-[600px] md:min-h-[75vh] flex items-center bg-[var(--ll-ink)] text-white overflow-x-hidden py-32">
      {bg && (
        ...
        <div className="max-w-7xl mx-auto px-[var(--ll-content-x)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-[var(--ll-cream)] rounded-[var(--ll-radius-md)] shadow-[var(--ll-shadow-card)] hover:shadow-[var(--ll-shadow-lift)] border border-[var(--ll-stone)] transition-all duration-300 p-6 flex flex-col justify-between group"
              >
                <div>
    ...
    <section className="py-10 sm:py-12 bg-gradient-to-r from-[#1b1b1b] to-neutral-800 text-white border-t border-neutral-700 overflow-x-hidden">
      <div className="max-w-5xl mx-auto px-6 text-center">
        <span className="ll-eyebrow block mb-4">Get Started Today</span>
          <h2 className="text-3xl md:text-5xl font-black tracking-[-0.03em] text-white mb-5">
          {content?.title || "Ready to Get Started?"}
        </h2>
        {content?.subtitle && (
          <p className="text-white/60 text-base md:text-lg max-w-2xl mx-auto mb-10 font-light">
            {content.subtitle}
          </p>
        )}
